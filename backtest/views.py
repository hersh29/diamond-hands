...
def crypto(request):
    ...
    if request.method == "POST":
        ...
        if f_form.is_valid():
            f_company = Stock.objects.get(symbol=f_form.cleaned_data["f_crypto"])
            ...
            context.update(
                {
                    "f_success": True,
                    "f_crypto": f_company.company_name,
        ...
        if "b_test" in request.POST:
            ...
            if b_form.is_valid():
                b_calc = Calculator(
                    b_form.cleaned_data["b_start_date"],
                    b_form.cleaned_data["b_end_date"],
                    b_form.cleaned_data["b_crypto"],
                    b_form.cleaned_data["b_amount"],
                    None,
                )
                b_company = Stock.objects.get(symbol=b_form.cleaned_data["b_crypto"])
                context.update(
                    {
                        "b_success": True,
                        "b_crypto": b_company.company_name,
                        "b_start_date": b_form.cleaned_data["b_start_date"],
                        "b_end_date": b_form.cleaned_data["b_end_date"],
                        "b_starting_amount": b_form.cleaned_data["b_amount"],
                        "b_ending_amount": b_calc.ending_amount(),
